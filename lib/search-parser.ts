...
export function parseSearchInput(input: string): SearchFilters {
  ...
  const cityPatterns = ["kathmandu", "pokhara", "bhaktapur", "lalitpur", "biratnagar", "patan"];
  ...
  const bedroomMatch = lowerInput.match(/(\d+)\s*(?:bhk|bedroom|bed|bed\s?room)/);
  if (bedroomMatch) {
    filters.bedrooms = parseInt(bedroomMatch[1]);
  }

  const priceMatch = lowerInput.match(/(?:rs\.?\s*|₹\s*)(\d+)\s*(?:to|-|and)\s*(?:rs\.?\s*|₹\s*)?(\d+)/i);
  if (priceMatch) {
  ...
  const amenityList = ["wifi", "water 24/7", "furnished", "parking", "garden", "balcony", "ac", "kitchen"];
  filters.amenities = amenityList.filter(amenity => lowerInput.includes(amenity));
